...
ARABIC_CASES: list[RetrievalCase] = [
    ...
    # ── السياسة التأديبية ────────────────────────────────────────────────────
    _case("متى صدر هذا الإصدار من السياسة التأديبية؟",
          "HC-PC-006-AR§6.0", reasoning_type=ReasoningType.TEMPORAL),
    _case("أنا أحاول لكني لا أستطيع أداء المطلوب — هل هذه مسألة تأديبية؟",
          "HC-PC-006-AR§6.1", "HC-PC-008-AR§8.1", reasoning_type=ReasoningType.COMPARATIVE),
    _case("أوقفوني عن العمل ريثما يُحقَّق معي، هل هذه عقوبة وهل أُدفع لي؟",
          "HC-PC-006-AR§6.2"),
    _case("تغيبت مرتين دون إذن، ماذا يحدث في المرة الثالثة؟",
          "HC-PC-006-AR§6.3", modality=Modality.TABLE),
    _case("ما الأفعال التي قد تؤدي إلى فصلي فوراً؟",
          "HC-PC-006-AR§6.4"),
    _case("لم ألتزم بأيام الحضور المتفق عليها، فما العاقبة؟",
          "HC-PC-006-AR§6.5"),
    _case("صدرت بحقي عقوبة، كم أمامي من الوقت للاعتراض؟",
          "HC-PC-006-AR§6.6", "HC-PC-009-AR§9.2", reasoning_type=ReasoningType.TEMPORAL),

    # ── التعريفات والدرجات ───────────────────────────────────────────────────
    _case("ما الذي تغيّر في هذه الوثيقة مؤخراً؟",
          "HC-PC-007-AR§7.0", reasoning_type=ReasoningType.TEMPORAL),
    _case("عقدي يقول شيئاً والسياسة تقول غيره، أيهما يُطبَّق؟",
          "HC-PC-007-AR§7.1", reasoning_type=ReasoningType.RELATIONSHIP),
    _case("أخذت إجازة بدون أجر شهرين، هل انقطعت خدمتي؟",
          "HC-PC-007-AR§7.2"),
    _case("حين تقولون خمسة أيام، هل تقصدون أيام الدوام أم بما فيها الجمعة والسبت؟",
          "HC-PC-007-AR§7.3", modality=Modality.TABLE),
    _case("هل تدخل مكافأة الأداء وبدل الإنترنت في حساب أجر الإجازة؟",
          "HC-PC-007-AR§7.4"),
    _case("ما معنى أن الفترة متحركة وليست سنة تقويمية؟",
          "HC-PC-007-AR§7.5", reasoning_type=ReasoningType.TEMPORAL),
    _case("درجتي السادسة، هل أسافر بدرجة رجال الأعمال وكم أعتمد من مصروفات؟",
          "HC-PC-007-AR§7.6", modality=Modality.TABLE),
    _case("وظيفتي تتطلب حضوري إلى الموقع، فتحت أي فئة تقع؟",
          "HC-PC-007-AR§7.7"),
    _case("ما الحد الأدنى من التقييم الذي يُعد مقبولاً؟",
          "HC-PC-007-AR§7.8", modality=Modality.TABLE),
    _case("أي نموذج أستخدم لطلب العمل عن بُعد ومتى يُعتمد؟",
          "HC-PC-007-AR§7.9", modality=Modality.TABLE),
    _case("سألت عن مكافأة نهاية الخدمة ولم أجد سياسة، لماذا؟",
          "HC-PC-007-AR§7.10"),

    # ── الجدارة وإدارة الأداء ────────────────────────────────────────────────
    _case("منذ متى تسري سياسة الجدارة؟",
          "HC-PC-008-AR§8.0", reasoning_type=ReasoningType.TEMPORAL),
    _case("هل تُعامل مشكلة الأداء معاملة المخالفة السلوكية؟",
          "HC-PC-008-AR§8.1", "HC-PC-006-AR§6.1", reasoning_type=ReasoningType.COMPARATIVE),
    _case("استنفدت رصيدي المرضي بالكامل، ما الذي تفعله الشركة بعد ذلك؟",
          "HC-PC-008-AR§8.2", "HC-PC-002-AR§2.5"),
    _case("حصلت على تقييم منخفض، فما الخطوة التالية وكم تستغرق؟",
          "HC-PC-008-AR§8.3", "HC-PC-007-AR§7.8"),
    _case("لم أوافق على نتيجة مراجعة أدائي، ماذا أفعل؟",
          "HC-PC-008-AR§8.4", "HC-PC-009-AR§9.2"),

    # ── التظلمات والاعتراضات ─────────────────────────────────────────────────
    _case("ما آخر تحديث طرأ على سياسة التظلمات؟",
          "HC-PC-009-AR§9.0", reasoning_type=ReasoningType.TEMPORAL),
    _case("ما الأمور التي يحق لي أن أشتكي منها أصلاً؟",
          "HC-PC-009-AR§9.1"),
    _case("مشكلتي مع مديري نفسه، فإلى من أرفعها وكم تستغرق؟",
